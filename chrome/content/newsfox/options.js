...
// need to set as load() gets called without save() when browser is opened
	...
	// 2 descending by #unread
		...
		this.htmlRemove = NFgetPref("advanced.htmlRemove", "str", "img,embed,svg,iframe").replace(/ /gi,"");
...
// https://developer.mozilla.org/En/Firefox_addons_developer_guide/Using_XPCOM%E2%80%94Implementing_advanced_processes
			{
				this.keywordColor.push(NFgetPref("keywordColor"+i, "str", "#008000"));
				this.keyword.push(decodeURIComponent(escape(NFgetPref("keyword"+i++, "str", null))));
			}
		this.keywordSearch = NFgetPref("advanced.keywordSearch", "bool", false);
		this.keywordGroup = NFgetPref("advanced.keywordGroup", "bool", false);
		this.keywordFeed = NFgetPref("advanced.keywordFeed", "bool", true);
	...
	this.save = function(savenonUI)
	{
		...
		NFsetPref("advanced.htmlRemove", "str", this.htmlRemove);
		...
		for (var i=1; i<=this.keyword.length; i++)
			{
				NFsetPref("keywordColor"+i, "str", this.keywordColor[i-1]);
		...
		while (NFgetPref("keyword"+i, "str", null))
		{
			prefs.clearUserPref("keywordColor"+i);
