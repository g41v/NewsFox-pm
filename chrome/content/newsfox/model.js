...
FeedGroup.prototype.hasKeyword = function(keyword)
	{
	if (gOptions.keywordGroup)
	{
		if (this.search && !this.searchTag)    // search group
		{
			if (gOptions.keywordSearch)
			{
				var srchitem = new Array();
				var srchstr = null;
				if (this.srchdat.text != "") srchstr = mkSrchstr(this.srchdat,srchitem);
				var now = new Date().getTime();
				for (var i=0; i<this.list.length; i++)
				{
					var feed = gFmodel.get(this.list[i]);
					for (var j=0; j<feed.size(); j++)
					{
						var article = feed.get(j);
						if (article.body.toLowerCase().indexOf(keyword) > -1 || article.title.toLowerCase().indexOf(keyword) > -1)
							if (hasPropSandbox.hasProp(j, feed, this.srchdat, srchstr, srchitem, now))
								return true;
					}
				}
				return false;
		...
		else if (this.search)    // tag group
		{
			...
			for (var i=0; i<gFdGroup[0].list.length; i++)
			{
				...
				for (var j=0; j<feed.size(); j++)
				{
					if (hasTag(j, feed, this.searchTag))
					{
						var article = feed.get(j);
						if (article.body.toLowerCase().indexOf(keyword) > -1 || article.title.toLowerCase().indexOf(keyword) > -1) return true;
					}
				}
			}
			return false;
		...
		{
			for (var i=0; i<this.list.length; i++)
				if (gFmodel.get(this.list[i]).hasKeyword(keyword,0)) return true;
		}
		return false;
	}
	else
...
function Feed()
{
	...
	this.hasKeyword = function(keyword, nCategory)
	{
	if (gOptions.keywordFeed)
	{
		if (nCategory == 0)
			for (var i=0; i<this.size(); i++)
			{
				var article = this.get(i);
				if (article.body.toLowerCase().indexOf(keyword) > -1 || article.title.toLowerCase().indexOf(keyword) > -1) return true;
			}
		else
			for (var i=0; i<this.size(); i++)
			{
				...
				var article = this.get(i);
					if (ScatS.indexOf(ScatnameS) > -1 && (article.body.toLowerCase().indexOf(keyword) > -1 || article.title.toLowerCase().indexOf(keyword) > -1))
						return true;
			}
		return false;
	}
...
//	this. rowCount getter = function() { return gIdx.fdgp.length; }
	...
	this.getRowProperties = function(row,props)
	{
		...
		for (var i=1; i<=gOptions.keyword.length; i++)
		{
			...
			if (nFeed == -1 && gFdGroup[gIdx.fdgp[row]].hasKeyword(gOptions.keyword[i-1]))
				hasKeyword = true;
			if (nFeed > -1 && gFmodel.get(nFeed).hasKeyword(gOptions.keyword[i-1],gIdx.catg[row]))
				hasKeyword = true;
			if (hasKeyword)
			{
				returnProps = propsAdd(""+KEYWORD+(i-1), props, returnProps);
...
// "Linux" on GNU/Linux; and "Darwin" on Mac OS X.
	...
	if (row.value > -1)
	{
		...
		evt.dataTransfer.setData("text/x-moz-url", art.link);
	...
	switch (col.value.id)
	{
		case "title":
			...
			else
			{
				...
				var p = getXhtmlBody(art.body,"span",doc,null,5,"img,embed,svg,iframe");
...
function ArticleTreeModel()
{
	...
	this.getRowProperties = function(row,props)
	{
		...
		for (var i=1; i<=gOptions.keyword.length; i++)
			if (gCollect.get(row).body.toLowerCase().indexOf(gOptions.keyword[i-1]) > -1 || gCollect.get(row).title.toLowerCase().indexOf(gOptions.keyword[i-1]) > -1)
			{
				returnProps = propsAdd(""+KEYWORD+(i-1), props, returnProps);
				returnProps = propsAdd(KEYWORD, props, returnProps);
			}
