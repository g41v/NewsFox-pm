...
function init()
{
	...
	document.getElementById("htmlRemoveBox").value = gOpt.htmlRemove;
	styleChange();
	...
	for (var i=0; i<Math.min(gOpt.keyword.length,5); i++)
		if (gOpt.keyword[i])
		{
			document.getElementById("color"+i).color = gOpt.keywordColor[i];
...
function doAccept()
{
	...
	gOpt.htmlRemove = document.getElementById("htmlRemoveBox").value.replace(/ /gi,"");
	...
	for (var i=gOpt.keyword.length; i<5; i++)
	{
		...
		var newColor = document.getElementById("color"+i).color;
		if (stringTrim(newValue) == "") continue;
		else
		{
			gOpt.keyword.push(newValue);
			gOpt.keywordColor.push(newColor);
		}
	}
	for (var i=lenToCheck; i>= 0; i--)
	{
		...
		{
			if (stringTrim(newValue) == "")
			{
				gOpt.keyword.splice(i,1);
				gOpt.keywordColor.splice(i,1);
			}
			else
			{
				gOpt.keyword[i] = newValue;
				gOpt.keywordColor[i] = newColor;
			}
		}
		else // only change color
			gOpt.keywordColor[i] = newColor;
	}
...
{
	var list = document.getElementById("style");
	var style = list.selectedIndex;
	if (style != 4)
	{
		document.getElementById("htmlRemoveLabel").disabled = true;
		document.getElementById("htmlRemoveBox").disabled = true;
	}
	else
	{
		document.getElementById("htmlRemoveLabel").disabled = false;
		document.getElementById("htmlRemoveBox").disabled = false;
	}
}
