...
const NF_startURL = "chrome://newsfox/content/newsfox.xul";
...
{
//	if (NFgetPref("browser.tabs.remote.autostart", "bool", false, true) || NFgetPref("browser.tabs.remote.autostart.1", "bool", false, true) || NFgetPref("browser.tabs.remote.autostart.2", "bool", false, true))
//		window.alert("NewsFox only works in non E10S mode\n\t(see Firefox general options)");
...
// modified from  https://developer.mozilla.org/en-US/Add-ons/Code_snippets/Tabbed_browser
	...
	while (!found && browserEnumerator.hasMoreElements())
	{
		...
		for (var index = 0; index < numTabs; index++)
		{
			...
			if (NF_startURL == currentBrowser.currentURI.spec)
			{
	...
	if (!found)
	{
	 // Now we are sure that no NewsFox is opened. Starting one.
		if (newTab)
			window.getBrowser().selectedTab = window.getBrowser().addTab(NF_startURL);
		else
			window.getBrowser().loadURI(NF_startURL, null, null);
	}
}
	...
	cleanup : function()
	{
		...
		if (noYesConfirm(NF_SB.getString('uninstallDeleteFilesPrefs')))
		{
			try
			{
				...
				if (noYesConfirm(NF_SB.getString('uninstallDeleteFolder') + "\n\n" + file.path))
